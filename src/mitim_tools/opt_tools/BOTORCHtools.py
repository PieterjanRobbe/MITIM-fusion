...
class ModifiedModelListGP(botorch.models.model_list_gp_regression.ModelListGP):
    ...
    def _batched_posterior(self, X, observation_noise=False):
        ...
        for i, m in enumerate(self.models):
            ...
            use_cache = not X.requires_grad
            if use_cache and (hasattr(tf1, "_cached_factor_X") and tf1._cached_factor_X is X):
                factor_i = tf1._cached_factor
            else:
                factor_i = tf1.surrogate_parameters["transformationOutputs"](
                    X, tf1.surrogate_parameters, tf1.output
                ).to(X.device)
                if use_cache:
                    tf1._cached_factor = factor_i.detach()
                    tf1._cached_factor_X = X
            factors_list.append(factor_i.squeeze(-1))  # (*orig_shape,)
...
class Transformation_Outcomes(botorch.models.transforms.outcome.Standardize):
    ...
    def untransform_posterior(self, X, posterior):
        if (self.output is not None) and (self.flag_to_evaluate):
            ...
            use_cache = not X.requires_grad
            if use_cache and (hasattr(self, "_cached_factor_X") and self._cached_factor_X is X):
                ...
                if use_cache:
                    self._cached_factor = factor.detach()
                    self._cached_factor_X = X

            self.stdvs = factor
            self.means = self.stdvs * 0.0
            self._stdvs_sq = self.stdvs.pow(2)
            posterior_tf = super().untransform_posterior(posterior)
            # Ensure model attributes remain leaf tensors so model.deepcopy() used by
            # BoTorch fantasizing (KG) does not fail.
            self.stdvs = self.stdvs.detach()
            self.means = self.means.detach()
            self._stdvs_sq = self._stdvs_sq.detach()
            return posterior_tf
